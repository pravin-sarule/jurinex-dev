"""
storage_policy.py — Storage quota enforcement for agent-draft-service.

Mirrors the logic in ChatModel/services/llmChatPolicyService.js assertStorageAllowed.
Returns {"ok": True} when upload is permitted, or
        {"ok": False, "code": "STORAGE_LIMIT_EXCEEDED", "message": "..."} when not.

Fails OPEN (allows upload) if any DB connection is unavailable, so a transient
DB outage does not permanently block users.
"""
...
def _doc_conn():
    ...
    if not url:
        raise ValueError("DOCUMENT_DATABASE_URL is not set")
...
def assert_storage_allowed(user_id: int | str, size_bytes: int = 0) -> Dict[str, Any]:
    """
    Check whether adding `size_bytes` would push the user over their plan's
    storage_limit_gb.  Returns {"ok": True} when allowed.
    """
    if not user_id:
        return {"ok": True}
    ...
    # ── 1. Current file storage used (Document_DB) ────────────────────────────
    ...
    # ── 2. Plan storage limit (Payment_DB) ───────────────────────────────────
    ...
    if used_after > storage_limit_bytes:
        ...
        return {
            ...
            "message": (
                ...
                f"This upload ({extra_gb:.3f} GB) would exceed your quota by "
                f"{overage / (1024**3):.3f} GB. "
