...
That row’s model is what `_generate_text(..., agent_name="form_population_agent")` uses. In this workspace it is **`gemini-3.7-flash`** at temperature **0.3**. Auto-fill always sends **`thinking_level=low`** for this agent only (`gemini-3.7-flash` does not support `minimal`; Low / Medium / High only).
