...
SERVICE_NAME = os.environ.get("SERVICE_NAME", "agentic-document-service")
...
def is_llm_consuming_request(method: str, path: str) -> bool:
    if method.upper() != "POST":
        return False
    p = path.rstrip("/")
    if "intelligent-chat" in p:
        return True
    if "/learning/" in p and any(k in p for k in ("generate", "analyze-relationships", "draft-bridge")):
        return True
    if "/sessions/" in p and p.endswith("/continue"):
        return True
    if "/cases/" in p and (p.endswith("/query") or ":execute" in p):
        return True
...
def _fallback_local_policy(user_id: int) -> dict[str, Any]:
    try:
        ...
        cfg = get_llm_chat_config(user_id=str(user_id), force_refresh=False, plan_limit_mode="summarization")
        check = assert_chat_allowed(str(user_id), cfg)
        if check.get("ok"):
            return {"ok": True, "source": check.get("source", "local_fallback"), "details": check}
        return {
            "ok": False,
            "code": check.get("code") or "TOKEN_LIMIT_EXHAUSTED",
            "message": check.get("message") or "Token limit reached.",
            "details": check.get("details") or check,
        }
    except Exception as exc:
        logger.warning("[PaymentTokenGuard] local fallback failed: %s", exc)
        if FAIL_OPEN:
            return {"ok": True}
        return {
            "ok": False,
    ...
    *,
    ...
    try:
        ...
        if resp.status_code == 404 or resp.status_code >= 500:
            logger.warning(
                "[PaymentTokenGuard] %s status=%s — using local policy fallback",
                url,
                resp.status_code,
            )
            return _fallback_local_policy(uid)
