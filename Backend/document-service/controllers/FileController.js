...
exports.completeSignedUpload = async (req, res) => {
  try {
    ...
    const limitCheck = { allowed: true, message: "Unlimited" };
...
exports.getChatCitations = async (req, res) => {
  try {
    ...
    const chatQuery = `
      ...
      FROM folder_chats
      WHERE id = $1::uuid AND user_id::text = $2::text AND folder_name = $3;
    `;
    const { rows: chatRows } = await pool.query(chatQuery, [chatId, String(userId), folderName]);
...
exports.deleteFolderChatSession = async (req, res) => {
  try {
    ...
    const checkResult = await pool.query(checkQuery, [String(userId), sessionId]);
    ...
    const result = await pool.query(deleteQuery, [String(userId), sessionId, normalizedFolderName]);
...
exports.deleteSingleFolderChat = async (req, res) => {
  try {
    ...
    const checkResult = await pool.query(checkQuery, [chatId, String(userId)]);
    ...
    const result = await pool.query(deleteQuery, [chatId, String(userId), normalizedFolderName]);
    const deletedCount = result.rowCount || 0;
...
exports.deleteAllFolderChats = async (req, res) => {
  try {
    ...
    const result = await pool.query(deleteQuery, [String(userId), normalizedFolderName]);
    const deletedCount = result.rowCount || 0;
...
exports.deleteFolderWithContents = async (req, res) => {
  ...
  try {
    ...
    await client.query(
      `DELETE FROM folder_chats WHERE user_id::text = $1::text AND folder_name = $2`,
