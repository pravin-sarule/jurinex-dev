...
async function callSinglePrompt(provider, prompt, systemPrompt, hasWebSearch = false, metadata = {}) {
        ...
        if (isGemini3Pro) {
          ...
          try {
            ...
            if (inputTokens === 0 && finalPrompt) {
              ...
              console.log(`⚠️ [Gemini 3.0 Pro FolderAI] API didn't return input tokens, calculated from text: ${inputTokens}`);
            }
            ...
            if (metadata && metadata.userId && modelName) {
              console.log(`🚀 [DEBUG FolderAI] Calling LLMUsageLogService.logUsage for Gemini 3.0 Pro`);
          ...
          if (inputTokens === 0 && prompt) {
            ...
            console.log(`⚠️ [Gemini Legacy FolderAI] API didn't return input tokens, calculated from text: ${inputTokens}`);
          }
          if (outputTokens === 0 && geminiResponse) {
          ...
          console.log(`🔍 [DEBUG FolderAI] Checking metadata for logging - userId: ${metadata?.userId}, modelName: ${modelName}, metadata:`, JSON.stringify(metadata));
          if (metadata && metadata.userId && modelName) {
            console.log(`🚀 [DEBUG FolderAI] Calling LLMUsageLogService.logUsage for Gemini legacy`);
            LLMUsageLogService.logUsage({
              userId: metadata.userId,
              modelName: modelName,
              inputTokens: inputTokens,
  ...
  const timeout = isClaude ? 120000 : 240000; // 2 minutes for Claude, 4 minutes for others
  ...
  if (inputTokens === 0 && prompt) {
    inputTokens = estimateTokenCount(prompt + (systemPrompt || ''));
    console.log(`⚠️ [${provider} FolderAI] API didn't return input tokens, calculated from text: ${inputTokens}`);
  }
  if (outputTokens === 0 && responseText) {
    outputTokens = estimateTokenCount(responseText);
    console.log(`⚠️ [${provider} FolderAI] API didn't return output tokens, calculated from text: ${outputTokens}`);
  }
  ...
  if (metadata && metadata.userId && resolvedModel) {
    console.log(`🚀 [DEBUG FolderAI] Calling LLMUsageLogService.logUsage for ${provider}`);
    LLMUsageLogService.logUsage({
      userId: metadata.userId,
...
async function* streamLLM(providerName, userMessage, context = '', relevant_chunks = null, originalQuestion = null) {
  ...
  console.log(`[Stream] 📤 Sending streaming request to ${provider} with max_tokens=${maxTokens}`);
          ...
          try {
            ...
            if (isClaude) {
              if (json.type === 'content_block_delta' && json.delta?.text) {
                ...
                text = json.content_block_delta.text;
              } else if (json.content_block?.text) {
            ...
            if (!line.startsWith('event:')) {
              console.warn(`[Stream] Failed to parse line: ${line.substring(0, 100)}`);
            }
          }
      ...
      for (const line of lines) {
        if (line.startsWith('data: ')) {
          ...
          try {
            const json = JSON.parse(data);
            const text = isClaude
              ? json.delta?.text || json.content_block_delta?.text || json.content_block?.text || ''
