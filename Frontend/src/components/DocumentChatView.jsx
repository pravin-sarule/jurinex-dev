...
const DocumentChatView = () => {
  ...
  const handleMessageClick = (message) => {
    ...
    setAnimatedResponseContent(response);
    setIsAnimatingResponse(false);
  };
              ...
              <div className="prose prose-gray max-w-none custom-markdown-renderer">
                <ReactMarkdown
                  ...
                  children={(() => {
                    ...
                    const messageResponse = selectedMessage?.answer || selectedMessage?.response || selectedMessage?.message || '';
                    
                    // Always prioritize the message's stored response
                    // Only use animatedResponseContent during active generation
                    // Only use currentResponse as a last resort
                    let rawResponse = '';
                    if (messageResponse) {
                      rawResponse = messageResponse;
                    } else if (animatedResponseContent) {
